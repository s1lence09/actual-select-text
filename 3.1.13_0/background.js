...
const SELECTEXT_SIGNIN_URL = `${WEBSITE_URL}/signin`;
const SELECTEXT_SIGNIN_WITH_GOOGLE_URL = `${WEBSITE_URL}/signin/initiate/oauth?method=google`;
...
browser.runtime.onMessage.addListener((request, sender) => {
  ...
  if (loginComplete !== undefined && loginComplete === true) {
    OCRAfterManualSignIn(sender.tab.id);
  }
  ...
  const signInAnotherWay = request["signInAnotherWay"];
  if (signInAnotherWay !== undefined && signInAnotherWay === true) {
    initiateSignInAnotherWay(sender.tab.id);
  }
...
async function OCRAfterManualSignIn(senderTabId) {
  const res = await browser.storage.local.get({ "cachedDataUri": "", "cachedUrl": "", "cachedTabId": "" })
  ...
  await browser.storage.local.set({ "cachedDataUri": "", "cachedUrl": "", "cachedTabId": "" })
}
...
function initiateManualSignin(activeTabId) {
  browser.tabs.create({ url: SELECTEXT_SIGNIN_WITH_GOOGLE_URL, openerTabId: activeTabId })
}


function initiateSignInAnotherWay(activeTabId) {
  browser.tabs.create({ url: SELECTEXT_SIGNIN_URL, openerTabId: activeTabId });
}
